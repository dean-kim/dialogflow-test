...
(function (root, factory) {
    ...
    const parseGIF = function (st, handler) {
        ...
        const parseExt = function (block) {
            ...
            const parseAppExt = function (block) {
                ...
                const parseTalkrExt = function (block) {
    ...
    const gestureController = function(start_frame, num_frames, tick_to_frame){
        const self = this;
        self.frame_indexes = [];
        ...
        self.construct_default_anim = function(default_delay, loop_end_delay, loop_end_delay_random){
            ...
            const frame_indexes_reversed = self.frame_indexes.slice().reverse();
            ...
            let calculated_loop_delay = default_delay;
            if( self.loop ){
    ...
    const SuperGif = function ( opts ) {
        ...
        const autoplay_blinks = options.autoplay_blinks;

        // Variables for play_for_duration
        let totalLipsyncAnimTime = 0;
        let playDuration = 0;
        let startTime = 0;
        let pingpong = false;
        ...
        let last_lipsync_frame = 0;

        let overrideFrameTime = null;
        ...
        const doShowProgress = function (pos, length, draw) {
            if (draw && showProgressBar) {
                ...
                if (options.is_vp) {
                    ...
                    if (false) {
                        if (!ctx_scaled) {
                            let l = options.vp_l, t = options.vp_t;
                            let w = options.vp_w, h = options.vp_h;
                        } else {
                            let l = options.vp_l/get_canvas_scale(), t = options.vp_t/get_canvas_scale();
                            let w = options.vp_w/get_canvas_scale(), h = options.vp_h/get_canvas_scale();
                        }
                        ctx.rect(l,t,w,h);
                        ctx.stroke();
                    }
        ...
        const pushFrame = function () {
            ...
            const delayTime = delay === 0 ? defaultFrameTime : delay;
            ...
            */
            if (currIdx > 0) {
                if (lastDisposalMethod === 3) {
                    ...
                    let bPreserveTalkrTransparency = false;
        ...
        const player = (function () {
            ...
            const setFrameIndexAndDirection = function() {
                // if ping-ponging, these are equivalent
                i = Math.abs(i);

                let num_lip_frames = Math.min(frames.length, last_lipsync_frame );

                let timeElapsed = Date.now() - startTime;
                let timeLeft = playDuration - timeElapsed;

                // We assume/enforce a constant per-frame time here.
                let perFrameTime = totalLipsyncAnimTime / num_lip_frames;
                if (overrideFrameTime) {
            ...
            const trigger_eyebrow_anim = function(duration){
                // if eyebrow channel exists
                if ('1' in talkr_channels){
                    if( Math.random() > 0.65 ) {
                        let eyebrows = talkr_channels['1'];
            ...
            const play_for_duration = function(duration, overrideFrameDuration, bPlayEyebrowAnim) {
        ...
        const initialize_talkr_channels = function(){
            ...
            if(talkr_channels[eyebrow_key]){
                let numframes = talkr_channels[eyebrow_key].numframes;
                let startframe = talkr_channels[eyebrow_key].index;
        ...
        let canvas, ctx, toolbar, tmpCanvas, eyebrowCanvas, blinkCanvas;
        let initialized = false;
        let load_callback = false;

        let eyebrowGestureController, blinkGestureController;
